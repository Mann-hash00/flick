...
## neura-terminal-kit
**Files:** `neura-terminal-kit/terminal.tsx` (kit) · `theme.ts` · `common.tsx` · `TwoStrategiesScene.tsx` · `ThreeLosingHabitsScene.tsx` · `MentalTrapsScene.tsx` · `ThreeIndicatorsScene.tsx` · `PayoutStatScene.tsx` · `ScarTissueScene.tsx` · `VISUAL-SYSTEM.md`
...
**Also covers:** indicator teaching. `ThreeIndicatorsScene.tsx` uses real `ema()`, `rsi()` and `atr()` computed from the displayed candles — an ATR/Keltner band whose width visibly tracks volatility with a stop sitting on it, a verified RSI divergence in an `RsiPane` beneath price, and EMA 20/50 warmed up on off-screen history so both lines span the visible window. Read the warm-up and claim-checking notes in `VISUAL-SYSTEM.md` before reusing these — both bit during the build.
**Also covers:** stat-and-product beats, and keeping a frame from reading as dull. `PayoutStatScene.tsx` holds a 100-dot population grid that survives three beats — it fills under a scan line, then half its lit dots turn grey and get struck through on screen (one population changing, not a new graphic), then it detonates with a shockwave and drifting debris. Underneath everything runs a permanent ambient layer: a drifting SVG dot lattice, motes, HUD corner brackets and an accent bloom on each beat boundary. Its claim-only beats each get a visual that earns the sentence — an evaluation bar that fills and stamps PASSED, an equity curve that climbs green, marks FUNDED at the top and gives it back in red. It also shows how to composite a supplied screen recording: a device frame sized so the capture is scaled *down*, a near-square corner radius (rounding ate the app's own tab rows), a slow accent sweep instead of an overlay pinned to scrolling UI, and label-only legend chips so nothing contradicts a mid-drag number on screen. Any figure it states is isolated in a named constant with a note on provenance — see the claim-checking rule in `VISUAL-SYSTEM.md`. Emoji are drawn as SVG, never typed: headless Chromium here has no emoji font.
**Also covers:** single-object narrative. `ScarTissueScene.tsx` carries one equity curve through a whole film rather than cutting between metaphors — it bows under a weight and snaps, resets clean, is whipped down while its earlier gain greys out behind the drop, gets sutured across the drawdown, then pulls back to reveal four identical scars behind it and one line still climbing. The pull-back scales x and y by different amounts (0.2 across, 0.62 down) the way a chart zooms out, because a uniform scale squashes the scars flat. It also holds the shatter technique: a drawing broken into real clipped wedges via `clipPath`, so every flying piece is a genuine fragment of the same artwork.
