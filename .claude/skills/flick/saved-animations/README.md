...
**Also covers:** stat-and-product beats. `PayoutStatScene.tsx` holds a 100-dot population grid that survives three beats — it fills, then half its lit dots turn grey on screen (one population changing, not a new graphic), then it detonates with a shockwave and drifting debris. It also shows how to composite a supplied screen recording: a device frame sized so the capture is scaled *down*, a slow accent sweep instead of an overlay pinned to scrolling UI, and a contrast lift so a dark app UI survives on a phone. Any figure it states is isolated in a named constant at the top of the file with a note on provenance — see the claim-checking rule in `VISUAL-SYSTEM.md`.
